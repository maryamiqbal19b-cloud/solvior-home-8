    ...
    <>
      ...
      <section className="relative bg-[#0B1B33] text-white px-6 md:px-10 py-28 text-center overflow-hidden">
        ...
        <div className="relative">
          <h1 className="font-display text-4xl md:text-5xl font-bold mb-5">Blog grid</h1>
      ...
      <section className="px-6 md:px-10 py-20 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {NEWS.map((post) => (
              <article key={post.slug} className="group border border-gray-200 bg-white overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <h3 className="font-display text-lg font-bold text-[#0B1B33] mb-4 leading-snug">
                    {post.title}
                  </h3>
                  <Link
                    href={`/blogs/${post.slug}`}
                    className="inline-flex items-center gap-3 bg-[#0B1B33] text-white pl-2 pr-6 py-2 rounded-full font-semibold hover:bg-[#1E3A8A] transition-colors group/btn w-fit"
                  >
                    <span className="w-9 h-9 rounded-full bg-[#2563EB] flex items-center justify-center shrink-0 transition-transform duration-300 group-hover/btn:translate-x-1">
                      <svg
